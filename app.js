(() => {
  ...
  function renderAccount() {
    ...
    $('linkedGames').innerHTML = state.games.map((game) => {
      ...
      const status = isLinked ? 'Verbunden' : (game.sso_ready ? 'Bereit zur ersten Anmeldung' : 'Spiel verfügbar');
  ...
  function renderGames() {
    ...
    for (const game of state.games) {
      ...
      article.innerHTML = `
        ...
        <div class="game-meta">
          <span class="pill">Online-Spiel</span>
          ${game.sso_ready ? '<span class="pill ready">Nadena ID verbunden</span>' : '<span class="pill">Nadena ID in Vorbereitung</span>'}
        </div>
        <button class="button play-button" type="button">${game.sso_ready ? 'Mit Nadena ID spielen' : 'Spiel öffnen'}</button>`;
      article.querySelector('button').addEventListener('click', () => startGame(game));
